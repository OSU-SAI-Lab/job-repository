...
This directory hosts a **training-free few-shot segmentation** system built on
**DINOv3 + SAM 3**: given 1–5 labeled support examples, it segments the target
object in a new query image with no training or fine-tuning (all models frozen).

```
support(image+mask) ──DINOv3 match──▶ prior map ──▶ SAM prompts ──▶ SAM 3 ──▶ final mask
```

➜ **The FSS library lives in [`fss/`](fss/README.md) — see its README for
architecture, install, the end-to-end example, and the `python -m fss` CLI.**

Quick start:

```bash
pip install -r requirements.txt
python -m fss --support img1.png:mask1.png --query q.png --out out.png --debug-out debug.png
```

---

## Legacy: Smart Labeller pipeline

The sections below document the original **Smart Labeller** code this directory was
adapted from — a multi-backend object classification/detection pipeline (OWLv2,
BioCLIP, DINOv3) that generates class-support embeddings, creates object proposals
via tiled inference, and performs few-shot classification via cosine/logit matching.
It is retained for reference; the FSS system above is the active work.
